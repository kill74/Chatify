...
use clifford::screen_share::{
    QualityPreset, ScreenShareManager, ScreenShareOptions, ScreenShareState,
};
...
use crossterm::event::{
    self as crossterm_event, DisableMouseCapture, EnableMouseCapture, Event, KeyCode, KeyEventKind,
    KeyModifiers,
};
...
fn footer_hint_line(state: &ClientState, width: usize) -> String {
    ...
    let is_sharing = state
        ...
        .map(|ss| {
            ss.session()
                .map(|s| s.state == ScreenShareState::Sharing)
                .unwrap_or(false)
        })
        .unwrap_or(false);
...
fn render_header_widget(frame: &mut ratatui::Frame, state: &ClientState, area: Rect) {
    ...
    let subtitle_text = format!(
        ...
        status_chip(
            "STATUS",
            format!("{} ({})", state.status.text, state.status.presence.as_str())
        ),
...
fn render_profile_panel(frame: &mut ratatui::Frame, state: &ClientState, area: Rect) {
    ...
    let lines = vec![
        Line::from(format!(
            "{} {} [{}]",
            state.status.avatar, state.me, state.client_id
        )),
        Line::from(format!(
            "status: {} ({})",
            state.status.text,
...
fn render_footer_widget(frame: &mut ratatui::Frame, state: &ClientState, area: Rect) {
    ...
    let footer = Paragraph::new(Line::from(vec![
        ...
        Span::styled(
            &state.input_buffer,
            Style::default().fg(state.theme.hint_color()),
        ),
    ]));
        ...
        match msg.msg_type {
            MessageType::Msg | MessageType::Dm => {
                ...
                if group_started {
                    ...
                    lines.push(Line::from(vec![Span::styled(header, dim_style)]));
                }
            ...
            MessageType::Img | MessageType::Audio => {
                let user = msg.user.as_deref().unwrap_or("?");
                let label = if msg.msg_type == MessageType::Img {
                    "IMG"
                } else {
                    "AUDIO"
                };
...
async fn handle_file_chunk_event(state: &SharedState, data: &JsonMap, ts: u64) {
    ...
    if let Some(transfer) = completed {
        ...
        let (msg_type, text) =
            if transfer.media_kind == MediaKind::Image && state_lock.media_enabled {
...
async fn handle_metrics_event(state: &SharedState, data: &JsonMap, ts: u64) {
    ...
    let conn_accepted = data
        .get("connections_accepted")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    let conn_closed = data
            ...
            match first {
                ...
                Some("start") => {
                    ...
                    for part in parts.iter().skip(1) {
                        match *part {
                            ...
                            _ if part.starts_with("source:") => {
                                options.source_id =
                                    Some(part.strip_prefix("source:").unwrap().to_string());
                            }
                            _ => {}
                        }
                    }
                    ...
                    if !state.voice_active {
                        state.add_notice(
                            "Join a voice room first with /voice before screen sharing.",
                        );
                        return CommandFlow::Continue;
                    }

                    state.add_notice(format!(
                        "Starting screen share (quality: {:?}, audio: {})...",
                        options.quality, options.audio
                    ));
                    enqueue_timed(
                        &state.ws_tx,
                        serde_json::json!({
